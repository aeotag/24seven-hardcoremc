...
void main() {
	...
	*/
	...
	if ( mc_Entity.x == ENTITY_VINES )
			position.xyz += calcMove(worldpos.xyz, 0.0020, 0.0032, 0.0021, 0.0017, 0.0018, 0.0020, vec3(0.5,0.1,0.5), vec3(0.2,0.1,0.2));
	#endif
	if (istopv > 0.9) {
	#ifdef WAVING_GRASS
	...
	if ( mc_Entity.x == ENTITY_LAVAFLOWING || mc_Entity.x == ENTITY_LAVASTILL ) {
			mat = 0.4;
			position.xyz += calcWaterMove(worldpos.xyz) * 0.055;
			}
	#endif
